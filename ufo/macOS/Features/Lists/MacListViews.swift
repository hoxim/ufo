...
struct MacListDetailView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            Log.msg("MacListDetailView appear. requestedListId=\(listId.uuidString) resolvedName=\(listName) itemCount=\(items.count) knownListIds=[\(knownIds)]")
            if list == nil {
                Log.error("MacListDetailView could not resolve list for listId=\(listId.uuidString)")
            }
        }
    }
