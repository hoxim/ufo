...
struct MacNoteEditorView: View {
    ...
    init(
        ...
        _richText = State(initialValue: MacNoteRichTextCodec.makeEditorText(from: note?.content ?? ""))
    ...
    private func applyBlockStyle(_ style: MacNoteBlockStyle) {
        let mutable = NSMutableAttributedString(attributedString: richText)
        let safeRange = sanitizedSelection(in: mutable)

        if mutable.length == 0 {
            let targetStyle: MacNoteBlockStyle = style == .body ? .body : style
            let seedText = MacNoteRichTextCodec.makeEditorText(from: targetStyle.markdownPrefix)
            richText = seedText
            selectedRange = NSRange(location: seedText.length, length: 0)
            return
        }

        let paragraphRanges = MacNoteRichTextCodec.paragraphRangesCovering(selection: safeRange, in: mutable)
        guard !paragraphRanges.isEmpty else { return }

        let currentStyles = paragraphRanges.map { MacNoteRichTextCodec.blockStyle(in: mutable, at: $0.location) }
        let targetStyle: MacNoteBlockStyle = currentStyles.allSatisfy { $0 == style } ? .body : style
        ...
        for paragraphRange in paragraphRanges.reversed() {
            ...
            MacNoteRichTextCodec.removeEditorPrefix(from: paragraph)
            if !targetStyle.editorPrefix.isEmpty {
                paragraph.insert(
                    NSAttributedString(
                        string: targetStyle.editorPrefix,
                        attributes: MacNoteRichTextCodec.attributes(for: targetStyle, bold: false, inlineCode: false, isPrefix: true)
                    ),
                    at: 0
                )
            }
            ...
            if hadNewline {
                paragraph.append(NSAttributedString(string: "\n", attributes: MacNoteRichTextCodec.attributes(for: targetStyle)))
            }
            ...
            MacNoteRichTextCodec.restyleParagraph(in: mutable, range: replacementRange, blockStyle: targetStyle)
    ...
    private func toggleInlineStyle(_ style: MacNoteInlineStyle) {
        let mutable = NSMutableAttributedString(attributedString: richText)
        let effectiveRange = MacNoteRichTextCodec.effectiveInlineRange(for: sanitizedSelection(in: mutable), in: mutable)
        guard effectiveRange.length > 0 else { return }

        let shouldEnable = !MacNoteRichTextCodec.isInlineStyleFullyEnabled(style, in: mutable, range: effectiveRange)
        MacNoteRichTextCodec.setInlineStyle(style, enabled: shouldEnable, in: mutable, range: effectiveRange)
    ...
    private func save() async {
        ...
        let markdownContent = MacNoteRichTextCodec.makeMarkdown(from: richText)
