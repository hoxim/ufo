...
enum MacNoteInlineStyle {
    case bold
    case inlineCode
}

enum MacNoteBlockStyle: String {
    case body
    case heading
    case bullet
...
enum MacNoteRichTextCodec {
    ...
    static func isInlineStyleFullyEnabled(_ style: MacNoteInlineStyle, in text: NSMutableAttributedString, range: NSRange) -> Bool {
    ...
    static func setInlineStyle(_ style: MacNoteInlineStyle, enabled: Bool, in text: NSMutableAttributedString, range: NSRange) {
        let key = style == .bold ? NSAttributedString.Key.noteBold : NSAttributedString.Key.noteInlineCode
        text.addAttribute(key, value: enabled, range: range)
        restyleParagraphsIntersecting(in: text, range: range)
    }

    static func restyleParagraph(in text: NSMutableAttributedString, range: NSRange, blockStyle: MacNoteBlockStyle) {
    ...
    static func blockStyle(in text: NSAttributedString, at location: Int) -> MacNoteBlockStyle {
        ...
        if let raw = text.attribute(.noteBlockStyle, at: clampedLocation, effectiveRange: nil) as? String,
           let style = MacNoteBlockStyle(rawValue: raw) {
            return style
        }

        let plain = text.string
        if plain.hasPrefix(MacNoteBlockStyle.bullet.editorPrefix) {
            return .bullet
        }
        if plain.hasPrefix(MacNoteBlockStyle.checklistUnchecked.editorPrefix) {
            return .checklistUnchecked
        }
        if plain.hasPrefix(MacNoteBlockStyle.quote.editorPrefix) {
            return .quote
        }
        return .body
    }
    ...
    static func removeEditorPrefix(from text: NSMutableAttributedString) {
        for style in [MacNoteBlockStyle.checklistUnchecked, .bullet, .quote] {
    ...
    static func attributes(for style: MacNoteBlockStyle, bold: Bool = false, inlineCode: Bool = false, isPrefix: Bool = false) -> [NSAttributedString.Key: Any] {
    ...
    private static func blockStyleAndContent(for line: String) -> (MacNoteBlockStyle, String) {
    ...
    private static func makeParagraph(content: String, style: MacNoteBlockStyle) -> NSAttributedString {
        let result = NSMutableAttributedString()
    ...
    private static func parseInlineMarkdown(_ content: String, style: MacNoteBlockStyle) -> NSAttributedString {
        let result = NSMutableAttributedString()
    ...
    private static func foregroundColor(for style: MacNoteBlockStyle) -> Any {
    ...
    private static func font(for style: MacNoteBlockStyle, bold: Bool, inlineCode: Bool, isPrefix: Bool) -> Any {
