    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingRoleCreator) {
            if let selectedSpace {
                NavigationStack {
                    MacRoleEditorScreen(spaceId: selectedSpace.id) {
                        loadCustomRoles()
                    }
                }
            }
        ...
        .sheet(item: $editingRole) { role in
            NavigationStack {
                MacRoleEditorScreen(role: role, spaceId: role.spaceId) {
                    loadCustomRoles()
                }
            }
        }
...
struct MacRoleEditorScreen: View {
    ...
    private func save() {
        ...
        do {
            ...
            Log.dbError("MacRolesScreen.saveRole", error)
        }
    }
}
