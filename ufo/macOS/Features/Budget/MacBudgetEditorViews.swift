...
#Preview("Mac Add Budget Entry") {
    NavigationStack {
        MacAddBudgetEntryView(
            store: macPreviewBudgetStore(),
            actor: UUID(),
            customCategories: ["Office", "Travel"]
        )
        .frame(width: 520, height: 620)
    }
}

#Preview("Mac Add Budget Goal") {
    NavigationStack {
        MacAddBudgetGoalView(
            store: macPreviewBudgetStore(),
            actor: UUID()
        )
...
#Preview("Mac Add Category Budget") {
    NavigationStack {
        MacAddCategoryBudgetView(
            initialCategory: "Home",
            initialAmount: 900,
            customCategories: ["Office"],
            existingCategories: [MacBudgetPresetCategory.food.title, MacBudgetPresetCategory.home.title, "Office"],
            onSave: { _, _ in },
            onAddCategory: { _ in }
        )
...
#Preview("Mac Add Custom Budget Category") {
    NavigationStack {
        MacAddCustomBudgetCategoryView { _ in }
            .frame(width: 420, height: 240)
    }
}

@MainActor
private func macPreviewBudgetStore() -> BudgetStore {
