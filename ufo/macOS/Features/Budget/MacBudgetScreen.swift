...
struct MacBudgetScreen: View {
    ...
    var body: some View {
        ...
        .adaptiveFormPresentation(isPresented: $showAddGoal) {
            if let budgetStore {
                MacAddBudgetGoalView(store: budgetStore, actor: authRepo.currentUser?.id)
                    .presentationDetents([.medium, .large])
            }
        }
        .adaptiveFormPresentation(isPresented: $showAddCategoryBudget) {
            MacAddCategoryBudgetView(
                initialCategory: nil,
                initialAmount: nil,
                customCategories: appPreferences.budgetCustomCategories,
        ...
        .adaptiveFormPresentation(isPresented: $showAddCustomCategory) {
            MacAddCustomBudgetCategoryView { value in
                appPreferences.addBudgetCustomCategory(value)
            }
    ...
    private var contentList: some View {
        List {
            ...
            if budgetStore != nil, !filteredEntries.isEmpty {
                MacBudgetCashFlowSection(entries: chartEntries)
            }

            MacBudgetCategoryBudgetsSection(
            ...
            MacBudgetCustomCategoriesSection(
            ...
            if !subscriptionEntries.isEmpty {
                MacBudgetSubscriptionsSection(entries: subscriptionEntries)
            }

            MacBudgetGoalsSection(
                goals: filteredGoals,
                onAddGoal: { showAddGoal = true }
            )

            MacBudgetTransactionsSection(
    ...
    private var availableCategories: [String] {
        let values = Set(MacBudgetPresetCategory.allCases.map(\.title) + appPreferences.budgetCustomCategories + (budgetStore?.entries ?? []).map(\.category))
    ...
    private var categorySummaries: [MacBudgetCategorySummary] {
        ...
        return categories
            .map { category in
                ...
                return MacBudgetCategorySummary(category: category, spent: spent, limit: limit)
            }
            .sorted { lhs, rhs in
