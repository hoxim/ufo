...
struct MacNotificationBellButton: View {
    let unreadCount: Int
...
struct MacHomeWidgetRow: Identifiable {
    enum Style {
        case single
        case pair
    }
...
struct MacHomeMetricCardPlaceholder: View {
    var body: some View {
...
struct MacActiveSpaceMenuButton: View {
    let space: Space
...
enum MacHomeRoute: Hashable, Identifiable {
    case missions
    case lists
    case notes
    case incidents
...
enum MacBudgetWidgetRange: CaseIterable {
    case today
    case week
    case month
...
#Preview("Mac Notification Bell") {
    VStack(spacing: 16) {
        MacNotificationBellButton(unreadCount: 0)
        MacNotificationBellButton(unreadCount: 3)
    }
    .padding()
    .background(AppTheme.Colors.canvas)
}

#Preview("Mac Active Space Button") {
    MacActiveSpaceMenuButton(
