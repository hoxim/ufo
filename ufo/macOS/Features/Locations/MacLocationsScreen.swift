...
struct MacLocationsScreen: View {
    ...
    @State private var locationViewModel: MacLocationViewModel
    ...
    init() {
        _locationViewModel = State(wrappedValue: MacLocationViewModel())
    }
    ...
    var body: some View {
        ...
        .onChange(of: spaceRepo.selectedSpace?.id) { _, newValue in
            Log.msg("MacLocationsScreen selected space changed to \(newValue?.uuidString ?? "nil")")
            Task {
