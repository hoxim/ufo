...
final class SharedListRepository {
    ...
    /// Deletes list locally by soft delete.
    func deleteListLocal(_ list: SharedList, actor: UUID?) throws {
        guard let context else { throw RepositoryError.missingLocalContext }
        list.deletedAt = .now
        list.updatedBy = actor
        list.updatedAt = .now
        list.version += 1
        list.pendingSync = true

        for item in list.items where item.deletedAt == nil {
            item.deletedAt = .now
