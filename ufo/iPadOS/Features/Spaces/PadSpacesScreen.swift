...
struct PadSpacesScreen: View {
    ...
    @State private var pendingAction: PadSpacePendingAction?
    @State private var selectedFilter: PadSpaceFilter = .all
    ...
    var body: some View {
        ...
        .sheet(item: $spaceToEdit) { space in
            PadSpaceEditorView(space: space)
        }
    ...
    private var filterBar: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                ForEach(PadSpaceFilter.allCases) { filter in
                    Button {
    ...
    private func perform(_ action: PadSpacePendingAction) async {
