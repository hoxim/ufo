...
struct PadBudgetScreen: View {
    ...
    @State private var selectedKindFilter: PadBudgetKindFilter = .all
    @State private var selectedRangeFilter: PadBudgetRangeFilter = .month
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItemGroup(placement: .primaryAction) {
                Menu {
                        Section("Period") {
                            ForEach(PadBudgetRangeFilter.allCases, id: \.self) { filter in
                                Button {
                        ...
                        Section("Type") {
                            ForEach(PadBudgetKindFilter.allCases, id: \.self) { filter in
                                Button {
        ...
        .adaptiveFormPresentation(isPresented: $showAddGoal) {
            if let budgetStore {
                PadAddBudgetGoalView(store: budgetStore, actor: authRepo.currentUser?.id)
                    .presentationDetents([.medium, .large])
            }
        }
        .adaptiveFormPresentation(isPresented: $showAddCategoryBudget) {
            PadAddCategoryBudgetView(
                initialCategory: nil,
                initialAmount: nil,
                customCategories: appPreferences.budgetCustomCategories,
        ...
        .adaptiveFormPresentation(isPresented: $showAddCustomCategory) {
            PadAddCustomBudgetCategoryView { value in
                appPreferences.addBudgetCustomCategory(value)
            }
    ...
    private var contentList: some View {
        List {
            ...
            if budgetStore != nil, !filteredEntries.isEmpty {
                PadBudgetCashFlowSection(entries: chartEntries)
            }

            PadBudgetCategoryBudgetsSection(
            ...
            PadBudgetCustomCategoriesSection(
            ...
            if !subscriptionEntries.isEmpty {
                PadBudgetSubscriptionsSection(entries: subscriptionEntries)
            }

            PadBudgetGoalsSection(
                goals: filteredGoals,
                onAddGoal: { showAddGoal = true }
            )

            PadBudgetTransactionsSection(
    ...
    private var availableCategories: [String] {
        let values = Set(PadBudgetPresetCategory.allCases.map(\.title) + appPreferences.budgetCustomCategories + (budgetStore?.entries ?? []).map(\.category))
    ...
    private var categorySummaries: [PadBudgetCategorySummary] {
        ...
        return categories
            .map { category in
                ...
                return PadBudgetCategorySummary(category: category, spent: spent, limit: limit)
            }
            .sorted { lhs, rhs in
