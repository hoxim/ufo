...
#Preview("iPad Add Budget Entry") {
    NavigationStack {
        PadAddBudgetEntryView(
            store: padPreviewBudgetStore(),
            actor: UUID(),
            customCategories: ["Travel", "Kids"]
        )
    }
}

#Preview("iPad Add Budget Goal") {
    NavigationStack {
        PadAddBudgetGoalView(
            store: padPreviewBudgetStore(),
            actor: UUID()
        )
    }
...
#Preview("iPad Add Category Budget") {
    NavigationStack {
        PadAddCategoryBudgetView(
            initialCategory: "Home",
            initialAmount: 1200,
            customCategories: ["Kids"],
            existingCategories: [PadBudgetPresetCategory.food.title, PadBudgetPresetCategory.home.title, "Kids"],
            onSave: { _, _ in },
            onAddCategory: { _ in }
        )
    }
...
#Preview("iPad Add Custom Budget Category") {
    NavigationStack {
        PadAddCustomBudgetCategoryView { _ in }
    }
}

@MainActor
private func padPreviewBudgetStore() -> BudgetStore {
