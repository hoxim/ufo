...
struct PadSavedPlacesMapView: View {
    ...
    private var annotations: [PadLocationAnnotation] {
        var items = places.map {
            PadLocationAnnotation(
                id: $0.id,
...
struct PadLocationsMapCard: View {
...
struct PadLocationPickerMap: View {
    @Binding var region: MKCoordinateRegion
    let places: [SavedPlace]
    let currentLocation: CLLocation?
    @State private var position: MapCameraPosition
...
private struct PadLocationAnnotation: Identifiable {
    let id: UUID
