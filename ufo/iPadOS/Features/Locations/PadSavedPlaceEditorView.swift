...
enum PadSavedPlaceInputMethod: String, CaseIterable, Identifiable {
    case address
...
struct PadAddSavedPlaceSheet: View {
    @Environment(\.dismiss) private var dismiss

    let viewModel: PadLocationViewModel
    let actorId: UUID?
    ...
    @State private var method: PadSavedPlaceInputMethod = .address
    @State private var addressQuery = ""
    @State private var latitudeText = ""
    @State private var longitudeText = ""
    @State private var selectedCoordinate: CLLocationCoordinate2D?
    @State private var selectedSource: PadSavedPlaceInputMethod?
    @State private var pickerRegion: MKCoordinateRegion
    @State private var isSaving = false
    @State private var activeSourceAction: PadSavedPlaceInputMethod?
    @State private var isWaitingForCurrentLocation = false
    @State private var showStylePicker = false

    init(
        viewModel: PadLocationViewModel,
        actorId: UUID?,
            ...
            Form {
                ...
                Section("locations.editor.section.location") {
                    Picker("locations.editor.field.inputMethod", selection: $method) {
                        ForEach(PadSavedPlaceInputMethod.allCases) { method in
    ...
    private func finalizeSelection(
        ...
        source: PadSavedPlaceInputMethod
    ) async {
    ...
    private func save() async {
        guard let selectedCoordinate else {
            ...
            Log.error("PadAddSavedPlaceSheet.save invalid coordinate. method=\(method.rawValue) title=\(title)")
            return
        }
        ...
        if let placeToEdit {
            Log.msg("PadAddSavedPlaceSheet.save updating placeId=\(placeToEdit.id.uuidString) title=\(cleanTitle)")
            ...
            Log.msg("PadAddSavedPlaceSheet.save creating title=\(cleanTitle) method=\(method.rawValue) selectedSpace=\(viewModel.locationStore?.currentSpaceId?.uuidString ?? "nil")")
        ...
        Log.msg("PadAddSavedPlaceSheet.save finished title=\(cleanTitle) savedPlace=\(savedPlace?.id.uuidString ?? "nil") lastError=\(viewModel.locationStore?.lastErrorMessage ?? "nil")")
        if let savedPlace {
...
private struct PadLocationSelectionSummary: View {
    let method: PadSavedPlaceInputMethod
    let address: String
