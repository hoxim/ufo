...
struct PadLocationsScreen: View {
    ...
    @State private var locationViewModel: PadLocationViewModel
    ...
    init() {
        _locationViewModel = State(wrappedValue: PadLocationViewModel())
    }
    ...
    var body: some View {
        ...
        .task {
            Log.msg("PadLocationsScreen.task start selectedSpace=\(spaceRepo.selectedSpace?.id.uuidString ?? "nil")")
        ...
        .onChange(of: spaceRepo.selectedSpace?.id) { _, newValue in
            Log.msg("PadLocationsScreen selected space changed to \(newValue?.uuidString ?? "nil")")
            Task {
