...
struct PadNoteRichTextEditorRepresentable: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        var parent: PadNoteRichTextEditorRepresentable

        init(_ parent: PadNoteRichTextEditorRepresentable) {
            self.parent = parent
        }
        ...
        func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
            ...
            let style = PadNoteRichTextCodec.blockStyle(in: mutable, at: paragraphRange.location)
            ...
            let contentText = PadNoteRichTextCodec.contentText(for: paragraph).trimmingCharacters(in: .whitespacesAndNewlines)

            if contentText.isEmpty {
                mutable.replaceCharacters(in: paragraphRange, with: NSAttributedString(string: "\n", attributes: PadNoteRichTextCodec.attributes(for: .body)))
            ...
            let insertion = NSMutableAttributedString(
                string: "\n" + style.editorPrefix,
                attributes: PadNoteRichTextCodec.attributes(for: style)
            )
            if style.editorPrefixLength > 0 {
                insertion.setAttributes(
                    PadNoteRichTextCodec.attributes(for: style, isPrefix: true),
            ...
            PadNoteRichTextCodec.restyleParagraphsIntersecting(in: mutable, range: affectedRange)
