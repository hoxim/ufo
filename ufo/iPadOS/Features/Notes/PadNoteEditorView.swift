...
struct PadNoteEditorView: View {
    ...
    init(
        ...
        _richText = State(initialValue: PadNoteRichTextCodec.makeEditorText(from: note?.content ?? ""))
    ...
    private func applyBlockStyle(_ style: PadNoteBlockStyle) {
        let mutable = NSMutableAttributedString(attributedString: richText)
        let safeRange = sanitizedSelection(in: mutable)

        if mutable.length == 0 {
            let targetStyle: PadNoteBlockStyle = style == .body ? .body : style
            let seedText = PadNoteRichTextCodec.makeEditorText(from: targetStyle.markdownPrefix)
            richText = seedText
            selectedRange = NSRange(location: seedText.length, length: 0)
            return
        }

        let paragraphRanges = PadNoteRichTextCodec.paragraphRangesCovering(selection: safeRange, in: mutable)
        guard !paragraphRanges.isEmpty else { return }

        let currentStyles = paragraphRanges.map { PadNoteRichTextCodec.blockStyle(in: mutable, at: $0.location) }
        let targetStyle: PadNoteBlockStyle = currentStyles.allSatisfy { $0 == style } ? .body : style
        ...
        for paragraphRange in paragraphRanges.reversed() {
            ...
            PadNoteRichTextCodec.removeEditorPrefix(from: paragraph)
            if !targetStyle.editorPrefix.isEmpty {
                paragraph.insert(
                    NSAttributedString(
                        string: targetStyle.editorPrefix,
                        attributes: PadNoteRichTextCodec.attributes(for: targetStyle, bold: false, inlineCode: false, isPrefix: true)
                    ),
                    at: 0
                )
            }
            ...
            if hadNewline {
                paragraph.append(NSAttributedString(string: "\n", attributes: PadNoteRichTextCodec.attributes(for: targetStyle)))
            }
            ...
            PadNoteRichTextCodec.restyleParagraph(in: mutable, range: replacementRange, blockStyle: targetStyle)
    ...
    private func toggleInlineStyle(_ style: PadNoteInlineStyle) {
        let mutable = NSMutableAttributedString(attributedString: richText)
        let effectiveRange = PadNoteRichTextCodec.effectiveInlineRange(for: sanitizedSelection(in: mutable), in: mutable)
        guard effectiveRange.length > 0 else { return }

        let shouldEnable = !PadNoteRichTextCodec.isInlineStyleFullyEnabled(style, in: mutable, range: effectiveRange)
        PadNoteRichTextCodec.setInlineStyle(style, enabled: shouldEnable, in: mutable, range: effectiveRange)
    ...
    private func save() async {
        ...
        let markdownContent = PadNoteRichTextCodec.makeMarkdown(from: richText)
...
#Preview("Note Editor - New") {
    let preview = PadNotesPreviewFactory.make()
