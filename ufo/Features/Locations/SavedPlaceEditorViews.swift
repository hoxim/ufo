...
enum SavedPlaceInputMethod: String, CaseIterable, Identifiable {
    case address
    ...
    var selectionTitle: String {
        switch self {
        case .address:
            return "Wybrano z adresu"
        case .currentLocation:
            return "Wybrano z bieżącej lokalizacji"
        case .coordinates:
            return "Wybrano z koordynatów"
        case .mapCenter:
            return "Wybrano z mapy"
        }
    }
}
...
struct AddSavedPlaceSheet: View {
    ...
    @State private var selectedSource: SavedPlaceInputMethod?
    @State private var pickerRegion: MKCoordinateRegion
    @State private var isSaving = false
    @State private var activeSourceAction: SavedPlaceInputMethod?
    ...
    private func save() async {
        ...
        if let placeToEdit {
            Log.msg("AddSavedPlaceSheet.save updating placeId=\(placeToEdit.id.uuidString) title=\(cleanTitle)")
            ...
            Log.msg("AddSavedPlaceSheet.save creating title=\(cleanTitle) method=\(method.rawValue) lat=\(selectedCoordinate.latitude) lon=\(selectedCoordinate.longitude) selectedSpace=\(viewModel.locationStore?.currentSpaceId?.uuidString ?? "nil")")
        ...
        Log.msg("AddSavedPlaceSheet.save finished title=\(cleanTitle) savedPlace=\(savedPlace?.id.uuidString ?? "nil") lastError=\(viewModel.locationStore?.lastErrorMessage ?? "nil")")
        if let savedPlace {
...
private struct LocationSelectionSummary: View {
    let method: SavedPlaceInputMethod
    let address: String
