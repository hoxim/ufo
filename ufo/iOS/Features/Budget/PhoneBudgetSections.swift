...
struct PhoneBudgetOverviewSection: View {
    let income: Double
    let expense: Double
    let balance: Double
...
struct PhoneBudgetCashFlowSection: View {
...
struct PhoneBudgetCategoryBudgetsSection: View {
    let summaries: [PhoneBudgetCategorySummary]
    let onAddLimit: () -> Void
    let onEdit: (PhoneBudgetCategorySummary) -> Void
    let onDelete: (IndexSet) -> Void

    var body: some View {
        Section {
            ForEach(summaries) { summary in
                PhoneBudgetCategoryBudgetRow(summary: summary)
...
private struct PhoneBudgetCategoryBudgetRow: View {
    let summary: PhoneBudgetCategorySummary
...
struct PhoneBudgetCustomCategoriesSection: View {
...
struct PhoneBudgetSubscriptionsSection: View {
    let entries: [BudgetEntry]

    var body: some View {
        Section("budget.view.section.subscriptions") {
            ForEach(entries) { entry in
                PhoneBudgetSubscriptionRow(entry: entry)
            }
        }
    }
}

private struct PhoneBudgetSubscriptionRow: View {
...
struct PhoneBudgetGoalsSection: View {
...
private struct PhoneBudgetGoalRow: View {
    let goal: BudgetGoal
...
struct PhoneBudgetTransactionsSection: View {
    let entries: [BudgetEntry]
    let onDelete: (IndexSet) -> Void
    ...
    var body: some View {
        Section("budget.view.section.transactions") {
            ForEach(entries) { entry in
                PhoneBudgetTransactionRow(entry: entry)
            }
            .onDelete(perform: onDelete)
        }
    }
...
private struct PhoneBudgetTransactionRow: View {
    let entry: BudgetEntry
