...
struct PhoneAddBudgetEntryView: View {
    ...
    private func categoryOptions(for kind: BudgetEntryKind) -> [String] {
        switch kind {
        case .income:
            return (PhoneBudgetPresetIncomeCategory.allCases.map(\.title) + customCategories).uniquedPreservingOrder()
        case .expense:
...
struct PhoneAddBudgetGoalView: View {
    @Environment(\.dismiss) private var dismiss
...
struct PhoneAddCategoryBudgetView: View {
    @Environment(\.dismiss) private var dismiss
    ...
    init(
        ...
        _selectedCategory = State(initialValue: initialCategory ?? existingCategories.first ?? PhoneBudgetPresetCategory.home.title)
...
struct PhoneAddCustomBudgetCategoryView: View {
    @Environment(\.dismiss) private var dismiss
