    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingRoleCreator) {
            if let selectedSpace {
                NavigationStack {
                    PhoneRoleEditorScreen(spaceId: selectedSpace.id) {
                        loadCustomRoles()
                    }
                }
            }
        ...
        .sheet(item: $editingRole) { role in
            NavigationStack {
                PhoneRoleEditorScreen(role: role, spaceId: role.spaceId) {
                    loadCustomRoles()
                }
            }
        }
...
struct PhoneRoleEditorScreen: View {
    ...
    private func save() {
        ...
        do {
            ...
            Log.dbError("PhoneRolesScreen.saveRole", error)
        }
    }
}
