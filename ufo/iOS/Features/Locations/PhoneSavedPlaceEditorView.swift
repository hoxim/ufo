...
enum PhoneSavedPlaceInputMethod: String, CaseIterable, Identifiable {
    case address
...
struct PhoneAddSavedPlaceSheet: View {
    @Environment(\.dismiss) private var dismiss

    let viewModel: PhoneLocationViewModel
    let actorId: UUID?
    ...
    @State private var method: PhoneSavedPlaceInputMethod = .address
    @State private var addressQuery = ""
    @State private var latitudeText = ""
    @State private var longitudeText = ""
    @State private var selectedCoordinate: CLLocationCoordinate2D?
    @State private var selectedSource: PhoneSavedPlaceInputMethod?
    @State private var pickerRegion: MKCoordinateRegion
    @State private var isSaving = false
    @State private var activeSourceAction: PhoneSavedPlaceInputMethod?
    @State private var isWaitingForCurrentLocation = false
    @State private var showStylePicker = false

    init(
        viewModel: PhoneLocationViewModel,
        actorId: UUID?,
    ...
    private func finalizeSelection(
        ...
        source: PhoneSavedPlaceInputMethod
    ) async {
    ...
    private func save() async {
        guard let selectedCoordinate else {
            ...
            Log.error("PhoneAddSavedPlaceSheet.save invalid coordinate. method=\(method.rawValue) title=\(title)")
            return
        }
        ...
        if let placeToEdit {
            Log.msg("PhoneAddSavedPlaceSheet.save updating placeId=\(placeToEdit.id.uuidString) title=\(cleanTitle)")
        ...
        Log.msg("PhoneAddSavedPlaceSheet.save finished title=\(cleanTitle) savedPlace=\(savedPlace?.id.uuidString ?? "nil") lastError=\(viewModel.locationStore?.lastErrorMessage ?? "nil")")
        if let savedPlace {
...
private struct PhoneLocationSelectionSummary: View {
    let method: PhoneSavedPlaceInputMethod
    let address: String
...
#Preview("Phone Add Saved Place") {
    ...
    return PhoneAddSavedPlaceSheet(
        viewModel: viewModel,
        actorId: user.id,
...
#Preview("Phone Edit Saved Place") {
    let schema = Schema([
        UserProfile.self,
        Space.self,
        SpaceMembership.self,
