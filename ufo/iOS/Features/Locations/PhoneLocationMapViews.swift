...
struct PhoneSavedPlacesMapView: View {
    ...
    private var annotations: [PhoneLocationAnnotation] {
        var items = places.map {
            PhoneLocationAnnotation(
                id: $0.id,
...
struct PhoneSavedPlaceRow: View {
...
struct PhoneLocationsMapCard: View {
...
struct PhoneLocationPickerMap: View {
    @Binding var region: MKCoordinateRegion
    let places: [SavedPlace]
    let currentLocation: CLLocation?
    @State private var position: MapCameraPosition
...
private struct PhoneLocationAnnotation: Identifiable {
    let id: UUID
