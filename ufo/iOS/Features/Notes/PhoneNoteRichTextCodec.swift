...
enum PhoneNoteInlineStyle {
    case bold
    case inlineCode
}

enum PhoneNoteBlockStyle: String {
    case body
    case heading
    case bullet
...
enum PhoneNoteRichTextCodec {
    ...
    static func isInlineStyleFullyEnabled(_ style: PhoneNoteInlineStyle, in text: NSMutableAttributedString, range: NSRange) -> Bool {
    ...
    static func setInlineStyle(_ style: PhoneNoteInlineStyle, enabled: Bool, in text: NSMutableAttributedString, range: NSRange) {
        let key = style == .bold ? NSAttributedString.Key.noteBold : NSAttributedString.Key.noteInlineCode
        text.addAttribute(key, value: enabled, range: range)
        restyleParagraphsIntersecting(in: text, range: range)
    }

    static func restyleParagraph(in text: NSMutableAttributedString, range: NSRange, blockStyle: PhoneNoteBlockStyle) {
    ...
    static func blockStyle(in text: NSAttributedString, at location: Int) -> PhoneNoteBlockStyle {
        ...
        if let raw = text.attribute(.noteBlockStyle, at: clampedLocation, effectiveRange: nil) as? String,
           let style = PhoneNoteBlockStyle(rawValue: raw) {
            return style
        }

        let plain = text.string
        if plain.hasPrefix(PhoneNoteBlockStyle.bullet.editorPrefix) {
            return .bullet
        }
        if plain.hasPrefix(PhoneNoteBlockStyle.checklistUnchecked.editorPrefix) {
            return .checklistUnchecked
        }
        if plain.hasPrefix(PhoneNoteBlockStyle.quote.editorPrefix) {
            return .quote
        }
        return .body
    }
    ...
    static func removeEditorPrefix(from text: NSMutableAttributedString) {
        for style in [PhoneNoteBlockStyle.checklistUnchecked, .bullet, .quote] {
    ...
    static func attributes(for style: PhoneNoteBlockStyle, bold: Bool = false, inlineCode: Bool = false, isPrefix: Bool = false) -> [NSAttributedString.Key: Any] {
    ...
    private static func blockStyleAndContent(for line: String) -> (PhoneNoteBlockStyle, String) {
    ...
    private static func makeParagraph(content: String, style: PhoneNoteBlockStyle) -> NSAttributedString {
        let result = NSMutableAttributedString()
    ...
    private static func parseInlineMarkdown(_ content: String, style: PhoneNoteBlockStyle) -> NSAttributedString {
        let result = NSMutableAttributedString()
    ...
    private static func foregroundColor(for style: PhoneNoteBlockStyle) -> Any {
    ...
    private static func font(for style: PhoneNoteBlockStyle, bold: Bool, inlineCode: Bool, isPrefix: Bool) -> Any {
