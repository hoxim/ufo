...
struct PhoneNotesScreen: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .primaryAction) {
                Button {
                    editorTarget = PhoneNoteEditorTarget()
                } label: {
            ...
            PhoneNoteDetailView(
                ...
                onEdit: {
                    ...
                    DispatchQueue.main.async {
                        editorTarget = PhoneNoteEditorTarget(note: note)
                    }
                }
            )
        }
    ...
    private var datedSections: [PhoneNoteDaySection] {
    ...
    private func loadReferences() {
        ...
        do {
            ...
            Log.dbError("PhoneNotesScreen.loadReferences (SwiftData fetch)", error)
            missions = []
            incidents = []
            people = []
            recentLocations = []
...
private struct PhoneNoteCardView: View {
    let note: Note
...
private struct PhoneNoteDaySection: Identifiable {
    let date: Date
    let notes: [Note]
...
private struct PhoneNoteEditorTarget: Identifiable, Hashable {
    let id = UUID()
    let note: Note?
