...
struct PhoneRoutinesScreen: View {
    @Environment(\.modelContext) private var modelContext
    @Environment(SpaceRepository.self) private var spaceRepo
    @Environment(AuthRepository.self) private var authRepo

    @State private var selectedDate = Date()
    @State private var selectedRange: PhoneRoutineRange = .day
        ...
        Group {
            if let selectedSpace = spaceRepo.selectedSpace {
                ...
                .adaptiveFormPresentation(isPresented: $showingCreator) {
                    PhoneRoutineEditorView(spaceId: selectedSpace.id, actorId: authRepo.currentUser?.id) {
                        loadData()
                    }
                    .id(creatorToken)
    ...
    private var rangePicker: some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 6) {
                ForEach(PhoneRoutineRange.allCases) { range in
                    Button {
                        selectedRange = range
                    } label: {
                        Text(range.title)
    ...
    private func loadData() {
        ...
        do {
            ...
            routines = []
            routineLogs = []
            Log.dbError("PhoneRoutinesScreen.loadData", error)
        }
    }
    ...
    private func logRoutine(_ routine: Routine, on date: Date) {
        ...
        do {
            ...
            Log.dbError("PhoneRoutinesScreen.logRoutine", error)
        }
    }
